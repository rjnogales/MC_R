...
ui <- (fluidPage(tabsetPanel((tabPanel("AÑO", 
                                      sidebarLayout(sidebarPanel(
                                       ...
                                       selectInput(inputId = "año_num_input1",
                                        ...
                                        ),mainPanel(selectInput(inputId = "Mesest",
                                                                label = "mes:",
                             ...
                             tabPanel("Vehiculos",
                                      sidebarLayout(sidebarPanel(
                                        ...
                                        ,width = 2),mainPanel(  selectInput(inputId = "Cuartilesf",
                                                                            label = "Cuartil:",
...
, "11014","31002","21004","31014","12055","31020","53018","12186","31039"
 )
...
server <- function(input, output) {  output$distPlot2 <- renderPlot({
  ...
  salidas_todas_diatotales<- salidas_todas_diatotales[order(salidas_todas_diatotales$`salidas/mill_24`,decreasing = T), ]
  names(salidas_todas_diatotales)<-c("Salidas","2023","2024","2025","kilometros23","kilometros24","kilometros25","salidas/mill_23","salidas/mill_24" ,"salidas/mill_25","COT")
  ...
  GIT<-as.data.table(cbind(row.names(salidas_todas_dia13),salidas_todas_dia13[,c(10:12)]))
  ...
  GIT<- GIT[order(GIT$`salidas/mill_24`,decreasing = T), ]
  names(GIT)<-c("Salidas","2023","2024","2025","kilometros23","kilometros24","kilometros25","salidas/mill_23","salidas/mill_24" ,"salidas/mill_25","COT")
  
  
    
    BYN<-as.data.table(cbind(row.names(salidas_todas_dia13),salidas_todas_dia13[,c(4:6)]))
    ...
    names(BYN)<-c("Salidas","2023","2024","2025","kilometros23","kilometros24","kilometros25","salidas/mill_23","salidas/mill_24" ,"salidas/mill_25","COT")

    
    ETM<-as.data.table(cbind(row.names(salidas_todas_dia13),salidas_todas_dia13[,c(7:9)]))
    ...
    names(ETM)<-c("Salidas","2023","2024","2025","kilometros23","kilometros24","kilometros25","salidas/mill_23","salidas/mill_24" ,"salidas/mill_25","COT")

    
    BCPC<-as.data.table(cbind(row.names(salidas_todas_dia13),salidas_todas_dia13[,c(1:3)]))
    ...
    names(BCPC)<-c("Salidas","2023","2024","2025","kilometros23","kilometros24","kilometros25","salidas/mill_23","salidas/mill_24" ,"salidas/mill_25","COT")
  ...
  mtc<-salidas_todas_diatotales[,c(1,2,3,4)]
  mtc<-data.frame(mtc)
  #mtc<-data.frame(colnames(t(mtc)),mtc)
  names(mtc)=c("Tipo","2023","2024","2025")
  mtc2<-mtc
  mtc1<-data.frame(t(mtc[,-1]))
  mtc<-mtc[,-1]
  mtc <- mtc[,c(input$año_num_input,input$año_num_input1)]
  mtctitulo<-salidas_todas_diatotales[1,11]
...
output$distPlot <- renderPlot({
  ...
  mtc<-salidas_todas_diatotales[,c(1,8,9,10)]
  mtc<-data.frame(mtc)
  #mtc<-data.frame(colnames(t(mtc)),mtc)
  names(mtc)=c("Tipo","2023","2024","2025")
  mtc2<-mtc
...
output$distPlot6 <- renderPlot({
 ...
 if(input$Totalr){
   ...
   legend(x = "top",bty = "n",horiz = T,inset =  c(-0.1,0.2), legend = paste( "kM",RUTASkm24, sep = "-") , fill = c("Gray","lightblue"),cex = 1.2)
   
   torta<-pie(t(RUTApor[,1]),main=paste( "Porcentaje(%) de Tipo de Salidas",input$Mesesr,"de" ,input$año_num_inputr,input$Rutas,sep = " "),labels= paste(RUTA1$Tipo,RUTApor[,1],"%",sep = " "), pty="s", col = rainbow(5))
...
output$distPlot7 <- renderPlot({
  ...
  salidavehiclo<-barplot(t(vehic2[,1]),main=paste( titulof,input$COTS,input$TIPOLOGIAs,input$Mesesf,"de",input$año_num_inputf,sep = " "),beside = T,names.arg = vehic1$vehiculo, cex.names=0.8,cex.axis = 1.2,cex.lab = 1.1, las=2,col = "yellow",ylim =c(0,(max(vehic2[,1])+15)))
...
output$distPlot8 <- renderPlot({
  ...
  legend(x = "top",bty = "n",horiz = T,inset =  c(-0.1,0.2), legend = paste( "kM",vehickm24, sep = "-") , fill = c("Gray","lightblue"),cex = 1.2)
  
  tortavehic<-pie(t(vehicpor[,1]),main=paste( "Tipo de Salidas %",input$Mesesf,"de" ,input$año_num_inputf,"vehiculo",input$VEHICULOS,sep = " "),labels= paste(vehic1$Tipo,vehicpor[,1],"%",sep = " "), pty="s", col = rainbow(5))
