...
Mes_valor<-data.table(unique(usos_funsionarioTodos[,8]))
Mes_valor[,Mes2:=Mes]
Mes_valor$Mes<-replace(Mes_valor$Mes,Mes_valor$Mes== "01","Enero")
Mes_valor$Mes<-replace(Mes_valor$Mes,Mes_valor$Mes== "02","Febrero")
Mes_valor$Mes<-replace(Mes_valor$Mes,Mes_valor$Mes== "03","Marzo")
Mes_valor$Mes<-replace(Mes_valor$Mes,Mes_valor$Mes== "04","Abril")
Mes_valor$Mes<-replace(Mes_valor$Mes,Mes_valor$Mes== "05","Mayo")
Mes_valor$Mes<-replace(Mes_valor$Mes,Mes_valor$Mes== "06","Junio")
Mes_valor$Mes<-replace(Mes_valor$Mes,Mes_valor$Mes== "07","Julio")
Mes_valor$Mes<-replace(Mes_valor$Mes,Mes_valor$Mes== "08","Agosto")
Mes_valor$Mes<-replace(Mes_valor$Mes,Mes_valor$Mes== "09","Septiembre")
Mes_valor$Mes<-replace(Mes_valor$Mes,Mes_valor$Mes== "10","Octubre")
Mes_valor$Mes<-replace(Mes_valor$Mes,Mes_valor$Mes== "11","Noviembre")
Mes_valor$Mes<-replace(Mes_valor$Mes,Mes_valor$Mes== "12","Diciembre")
...
usos_funsionarioTodos<-unique(usos_funsionarioTodos)
...
ui <- fluidPage((
  ...
  #(tabPanel("ATIPICOS POR PRODUCTO", 
...
tabPanel("USOS Y ATIPICOS POR OFICINA", 
             ...
             ,mainPanel(plotOutput("distPlot2"),
                        ...
                        downloadButton("dwd","Dowload Grafi"),width = 10,
                        
                        
                        
                        plotOutput("distPlot4"),
                        
                        fluidRow(column(1,selectInput(inputId = "Años2",
                                    label = "Años:",
                                    
                                    choices = list( "2026", "2025","2024"))),
                        
                                 column(2, selectInput(inputId = "Mes2",label = "mes2:",
...
server <- function(input, output, session) {  
  ...
  observeEvent(input$reload_all, {
    ...
    # 1) Relee/actualiza tus datos (ahora sí con load)
    load(file.path(current_dir, "Funsion_ati_final.RData"))
    load(file.path(current_dir, "tarjetas1.RData"))
    load(file.path(current_dir, "usos_funsionario.RData"))
...
output$distPlot4 <- renderPlot({
  ...
  #Funsion_ati_final1<-Funsion_ati_final1[Funsion_ati_final1$Oficina1=="BNM"]
  Funsion_ati_final1<-Funsion_ati_final1[Funsion_ati_final1$Oficina1==input$Oficinast]
  Funsion_ati_final1<-Funsion_ati_final1[Funsion_ati_final1$Diatipo==input$TIPODIAt]
  
  Funsion_ati_final2<-Funsion_ati_final1[,Funsio_mes:=paste(Nombre,Año,Mes,sep = "_")]
  #Funsion_ati_final2<-Funsion_ati_final2[Funsion_ati_final2$Año==input$Añost]
  
  
  
  
  Tipo_fun<-data.frame(tapply(Funsion_ati_final2$Qpax,Funsion_ati_final2[,c("Funsio_mes","Oficina1")],sum))
  nombres_funs<-data.table(colnames(t(Tipo_fun)))
  names(nombres_funs)="Funsionario"
  Tipo_fun<-cbind(nombres_funs,Tipo_fun)
  Tipo_fun<-replace(Tipo_fun,is.na(Tipo_fun),0)
 
  maximo<-max(Tipo_fun[,2])
  
  Tipo_fun<-cbind(str_split_fixed(Tipo_fun$Funsionario,"_",3),Tipo_fun)
  
  Tipo_fun1<-Tipo_fun[Tipo_fun$V3==input$Meses]
  Tipo_fun1<-Tipo_fun1[Tipo_fun1$V2==input$Añost]
  
  Tipo_fun2<-Tipo_fun[Tipo_fun$V3==input$Mes2]
  Tipo_fun2<-Tipo_fun2[Tipo_fun2$V2==input$Años2]
  
  setkey(Tipo_fun1,V1)
  setkey(Tipo_fun2,V1)
  
  
  Tipo_fun<- Tipo_fun2[Tipo_fun1,all=T]
  
  Tipo_fun<-Tipo_fun[,c(1,5,9)]
  names(Tipo_fun)=c("V1","M1","M2")
  Tipo_fun<-replace(Tipo_fun,is.na(Tipo_fun),0)
  Tipo_fun<- Tipo_fun[order(Tipo_fun[,3],decreasing = T), ]
  
  Funsionariox<-barplot(t(Tipo_fun[,c(2,3)]),main = paste((input$Oficinast),"Atipicos de",input$Mes2,input$Años2,"VS",input$Meses,"de",input$Añost,input$TIPODIAt,sep = " "),cex.axis = 1.2,names.arg = Tipo_fun$V1, col = c("pink","red"),beside = TRUE,las=2,ylim = c(0,(maximo)))
  text(Funsionariox, (t(Tipo_fun[,c(2,3)]))+2 ,labels = t(Tipo_fun[,c(2,3)]))
  legend(x = "topright",bty = "n",lty = 1,horiz = F, legend = paste(Tipo_fun$V1,Tipo_fun$M2, sep = " "), col= "red",cex = 0.8,xpd = TRUE)
  legend(x = "top",bty = "n",horiz = T, legend = c(paste(input$Mes2,input$Años2,sep = " " ),paste(input$Meses,input$Añost,sep = " " )), fill= c("pink","red"),cex = 1.1,xpd = TRUE)
...
output$dwd<-downloadHandler(filename = function(){
  ...
  paste("Atipico",input$Añost,input$TIPODIAt,input$Oficinast,input$Meses,input$Tipo,sep = ".")
  },
  
  content = function(file){
    
  if(input$Tipo=="png") {png(file)} 
...
output$Funsionario <- DT::renderDataTable(
  ...
  DT::datatable({Funsion_ati_final1},  filter = "top",  options = list(lenghtMenu= list(c(7,15,-1), c(5,15, "All")), pagelenght= 15))
  
)

output$funsionario1 <- DT::renderDataTable(
  
  DT::datatable({usos_funsionarioTodos},filter = "top", extensions = "Buttons",
